...
df <-read.csv(file="C:\\Users\\Chris\\Desktop\\HR_NumQuick.csv", 
              header = T, 
              check.names=F, 
...
# Input: Select what to display
    ...
    selectInput('ycol', 'Dependent Variable', "", selected = ""),
    checkboxGroupInput('xcol', 'Independent Variable(s)', "")
...
# Output: Tabset w/ correlation, summary, and table ----
      tabsetPanel(type = "tabs",
        tabPanel("Graphs", 
                 uiOutput("graph")),
        tabPanel("Data", 
                 DT::dataTableOutput("table"),style = "overflow-y: scroll;overflow-x: scroll;"),
        tabPanel("Summary",
...
server <- function(input, output, session) {
  ...
  #mlt <- reactive({multinom(reformulate(input$group))})
  ...
  output$p <- renderPlot({eval(expr)
    ...
    if(input$plot.type=="boxplot")	{		#control for 1D or 2D graphs
      ...
      {
        p<-p+ geom_point(aes_string(color=input$xcol),alpha=0.5, position = 'jitter',show.legend = F)
      }
